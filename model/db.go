...
type Comment struct {
	...
	VideoID    uint   `gorm:"index:not null;comment:评论视频的id"`
	Content    string `gorm:"type:varchar(191);not null;comment:评论的内容"`
	ActionType int    `gorm:"type:ENUM('1', '2');not null;comment:评论行为，1表示已发布评论，2表示删除评论"`
}

// 点赞表
type Like struct {
	BaseModel
	UserID  uint `gorm:"index:not null;comment:点赞用户id"`
	VideoID uint `gorm:"index:not null;comment:点赞视频id"`
	Liked   int  `gorm:"not null;default:1;comment:默认1表示已点赞，0表示未点赞"`
}
...
type Relation struct {
	...
	FollowingID uint `gorm:"index:not null;comment:user id关注的用户id"`
	Followed    int  `gorm:"not null;default:0;comment:默认0表示未关注，1表示已关注"`
}
