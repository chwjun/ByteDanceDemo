...
func processTask(task Task) Result {
	...
	case 1:
		...
		err := util.UpdateLikeCounts(task.UserID, task.VideoID, true)
		if err != nil {
			return Result{}
		}
	case 2:
		err = unlikeVideo(task.UserID, task.VideoID)
		err := util.UpdateLikeCounts(task.UserID, task.VideoID, false)
		if err != nil {
			return Result{}
		}
	default:
...
func unlikeVideo(userID uint, videoID uint) error {
	// 开始一个新的事务
	tx := dao.DB.Begin()
	if tx.Error != nil {
		return tx.Error
