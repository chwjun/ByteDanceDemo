...
func UpdateLikeCounts(userID uint, videoID uint, like bool) error {
	// 定义哈希表的键
...
func GetVideosLikes(videoIDs []uint) (map[uint]int64, error) {
	ctx := GlobalRedisClient.ctx
	pipe := GlobalRedisClient.client.Pipeline()

	futures := make(map[uint]*redis.StringCmd)
...
func GetUserFavorites(userIDs []uint) (map[uint]int64, error) {
	ctx := GlobalRedisClient.ctx
	pipe := GlobalRedisClient.client.Pipeline()
...
func GetTotalVideosLikes(videoIDs []uint) (int64, error) {
	ctx := GlobalRedisClient.ctx
	pipe := GlobalRedisClient.client.Pipeline()
